...
# Keywords to filter Polymarket events (keep finance/politics/economy/crypto only)
POLYMARKET_INCLUDE_KW = [
    # Finance & economy
    "stock", "bond", "treasury", "yield", "interest rate", "inflation", "recession",
    "gdp", "cpi", "fed", "rate cut", "rate hike", "tariff", "trade war", "debt ceiling",
    "default", "shutdown", "fiscal", "monetary", "central bank", "jobs report", "unemployment",
    "s&p", "nasdaq", "dow", "market", "ipo", "earnings",
    # Crypto
    "bitcoin", "btc", "ethereum", "eth", "crypto", "solana", "sol", "xrp",
    "stablecoin", "defi", "token", "blockchain", "halving", "etf",
    # Commodities
    "oil", "gold", "silver", "commodity",
    # Politics & geopolitics (affects markets)
    "trump", "biden", "president", "election", "congress", "senate", "governor",
    "war", "ceasefire", "sanction", "nato", "china", "russia", "ukraine", "iran",
    "israel", "netanyahu", "xi jinping", "putin", "eu ", "european union",
    "legislation", "bill sign", "executive order", "impeach",
]

POLYMARKET_EXCLUDE_KW = [
    "nfl", "nba", "mlb", "nhl", "ufc", "mma", "boxing", "premier league",
    "champions league", "fifa", "world cup", "tennis", "golf", "cricket",
    "formula 1", "f1 ", "nascar", "esport", "lol:", "valorant", "dota",
    "bachelor", "love island", "survivor", "big brother", "oscars", "grammy",
    "weather", "temperature", "hurricane", "earthquake",
    "tweets", "followers", "subscribers", "tiktok", "youtube views",
]
...
FINANCE_PRO_SITES = {"wallstreetcn", "jin10", "cls", "eastmoney", "gelonghui", "blockbeats", "polymarket"}
...
def fetch_polymarket(session: requests.Session, now: datetime) -> list[RawItem]:
    """Polymarket prediction markets — Gamma API (public, no auth).
    Fetches high-volume events and filters to finance/politics/economy/crypto."""
    items: list[RawItem] = []
    try:
        resp = session.get(
        ...
        for ev in events:
            ...
            if any(kw in title_lower for kw in POLYMARKET_EXCLUDE_KW):
                continue

            # Must match at least one finance/politics keyword
            if not any(kw in title_lower for kw in POLYMARKET_INCLUDE_KW):
                continue
            ...
            # Parse ALL markets/outcomes with probabilities
            ...
            for m in markets:
                ...
                try:
                    ...
                    q = str(m.get("question") or m.get("groupItemTitle") or "").strip()
            ...
            # Build display title — keep pure English for translation pipeline
            # Probability data is in meta.outcomes, not in title
            display_title = title

            # Use current time as published_at — these are live active markets,
            # startDate/createdAt is when the event was first created (often months ago)
            published = now

            items.append(RawItem(
                site_id="polymarket", site_name="Polymarket", source="预测市场",
                title=display_title, url=url, published_at=published,
                meta={
...
def collect_all(session: requests.Session, now: datetime) -> tuple[list[RawItem], list[dict[str, Any]]]:
    tasks = [
        ...
        ("polymarket", "Polymarket", fetch_polymarket),
    ]
...
def main() -> int:
    ...
    # ── Polymarket: translate outcome labels & questions ──
    poly_translate_cache: dict[str, str] = {}
    poly_translated_count = 0
    POLY_TRANSLATE_MAX = 200  # max new translations per run

    # Short labels that should NOT be translated (prediction market terms)
    POLY_SKIP_LABELS = {
        "yes", "no", "over", "under", "other", "none",
        "higher", "lower", "up", "down",
    }

    def poly_translate(text: str, is_label: bool = False) -> str:
        nonlocal poly_translated_count
        if not text or has_cjk(text):
            return text
        # Skip short generic prediction market labels
        if is_label and text.strip().lower() in POLY_SKIP_LABELS:
            return text
        if text in poly_translate_cache:
            return poly_translate_cache[text]
        # Check title_cache too (reuse existing translations)
        if text in title_cache:
            poly_translate_cache[text] = title_cache[text]
            return title_cache[text]
        if poly_translated_count >= POLY_TRANSLATE_MAX:
            return text
        tr = translate_to_zh_cn(session, text)
        if tr:
            poly_translate_cache[text] = tr
            poly_translated_count += 1
            return tr
        return text

    for items_list in [latest_items_finance, latest_items_all]:
        for item in items_list:
            if item.get("site_id") != "polymarket":
                continue
            meta = item.get("meta")
            if not isinstance(meta, dict):
                continue
            ...
            for o in outcomes:
                if isinstance(o, dict):
                    lbl = o.get("label", "")
                    if lbl and not has_cjk(lbl):
                        o["label_zh"] = poly_translate(lbl, is_label=True)
                    q = o.get("question", "")
                    if q and not has_cjk(q):
                        o["question_zh"] = poly_translate(q)
