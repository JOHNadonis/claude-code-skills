...
_TEAM_MODE=$(~/.Codex/skills/gstack/bin/gstack-config get team_mode 2>/dev/null || echo "false")
...
```bash
cd "$_ROOT"
git rm -r --cached .Codex/skills/gstack/ 2>/dev/null || true
if ! grep -qF '.Codex/skills/gstack/' .gitignore 2>/dev/null; then
  echo '.Codex/skills/gstack/' >> .gitignore
fi
rm -rf "$LOCAL_GSTACK"
```
