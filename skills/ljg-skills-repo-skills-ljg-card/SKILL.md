---
name: ljg-skills-repo-skills-ljg-card
...
```bash
node ~/.Codex/skills/ljg-card/assets/capture.js <html> <png> <width> <height> [fullpage]
```

依赖：`~/.Codex/skills/ljg-card/node_modules/` 中的 playwright。如报错：

```bash
cd ~/.Codex/skills/ljg-card && npm install playwright && npx playwright install chromium
```
