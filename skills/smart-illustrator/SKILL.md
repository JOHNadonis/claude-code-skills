...
/smart-illustrator --mode cover --platform youtube --topic "Codex 4 深度评测"
```
...
```bash
npx -y bun ~/.Codex/skills/smart-illustrator/scripts/mermaid-export.ts \
...
```bash
npx -y bun ~/.Codex/skills/smart-illustrator/scripts/excalidraw-export.ts \
...
# Step 2: 调用脚本
GEMINI_API_KEY=$GEMINI_API_KEY npx -y bun ~/.Codex/skills/smart-illustrator/scripts/generate-image.ts \
